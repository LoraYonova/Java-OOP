...
public abstract class BaseHouse implements House {
    ...
    public String getStatistics() {
        ...
        out.append(String.format("%s %s:", name, getClass().getSimpleName()));
        out.append(System.lineSeparator());

        if(cats.isEmpty()) {
            out.append("Cats: none");
        } else {
            List<String> collect = getCats().stream().map(Cat::getName).collect(Collectors.toList());
            out.append(String.format("Cats: %s", String.join(" ", collect)));
        }
